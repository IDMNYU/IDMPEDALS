...
* **knob3_pitch** controls the center frequency of the filter; as with the lowpass example above, its value is initially expressed as a MIDI pitch as an easy way to get pitch-linear scaling on the knob control. After smoothing with an **oopsy.ctrl.smooth3** abstraction, it is converted to frequency with the **mtof** operator.
...
This pedal implements an *octave fuzz* such as found in the [Octavia](https://en.wikipedia.org/wiki/Octavia_(effects_pedal)) pedal designed by [Roger Mayer](https://en.wikipedia.org/wiki/Roger_Mayer_(engineer)) for [Jimi Hendrix](https://en.wikipedia.org/wiki/Jimi_Hendrix). An octave fuzz uses a [rectifier](https://en.wikipedia.org/wiki/Rectifier) circuit to fold the negative side of the AC waveform over into the positive range; this has the effect of doubling the frequency of the input signal, raising the fundamental by an octave. This type of pitch shifting through full-wave rectification works, though the sound can be fairly rough due to slope of the signal near the fold-over point. The output of the rectified signal is then put into a fuzz circuit.

Our digital *octavia* pedal has two parameters: **knob3_oct** controls the blend of the octave effect with the dry signal. The octave effect is accomplished by using the **abs** operator (to rectify the signal) and the **dcblock** operator (to rebalance the rectified signal into the AC range). The blending of the dry and octave signal is done through an [equal-power fade](https://en.wikipedia.org/wiki/Fade_(audio_engineering)). 
...
Finally, the phaser effect uses [feedback](https://en.wikipedia.org/wiki/Feedback) to create a more resonant effect - the output of the allpass is multiplied by 0.7, delayed by one sample, and added back into the filter. This creates a more pronounced "whooshing" phasing effect, akin to classic phaser pedals.
...
This pedal implements a technique used by [Pete Townshend](https://en.wikipedia.org/wiki/Pete_Townshend) on the [ARP2600](https://en.wikipedia.org/wiki/ARP_2600) synthesizer in the 1970s, where he would use the *amplitude* of his guitar (via the ARP's [envelope follower](https://en.wikipedia.org/wiki/Envelope_detector)) to control the *frequency* of a [sine wave](https://en.wikipedia.org/wiki/Sine_wave). The louder he played, the higher the sine wave's frequency. The rate at which the sine wave updates its frequency is controlled by a [sample-and-hold](https://en.wikipedia.org/wiki/Sample_and_hold) circuit where the trigger speed is determined by the **knob4_rate** parameter. The **knob3_input** parameter controls the scaling level of the input signal, and **sw5** multiplies the sine with the raw input signal, creating a [ring modulation](https://en.wikipedia.org/wiki/Ring_modulation) effect (see below).
...
A [ring modulator](https://en.wikipedia.org/wiki/Ring_modulation) is a very early modulation technique where a carrier signal is multiplied by a modulating signal (normally a fixed oscillator). The output is the sum and differences of the two sources frequency content; this allows for easily creating unpitched, inharmonic, robotic, or metallic sounds from harmonic instruments. In analog circuitry, ring modulation is accomplished using a network of [diodes](https://en.wikipedia.org/wiki/Diode) in a loop, or *ring*-style arrangement. Using DSP, ring modulation can be accomplished using a simple signal multiplication - the <b>*</b> operator in **gen~** does the trick.
	
Our ring modulator pedal has one parameter - the frequency of the modulating oscillator set by **knob3_rate**. The input signal is then multiplied with this oscillator (a [sine wave](https://en.wikipedia.org/wiki/Sine_wave) generated by the **cycle** operator). Low frequencies will sound like a [tremolo](https://en.wikipedia.org/wiki/Tremolo) effect - as the frequencies raise above 20Hz, the [sideband frequencies](https://en.wikipedia.org/wiki/Sideband) of the ring modulation become audible.
